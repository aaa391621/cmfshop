...
class FavoritesController extends RestBaseController
{
    ...
    public function setFavorites()
    {
        ...
        //组装数据
        $data = $this->_FavoritesObject($input['title'], $input['url'], $input['description'], $input['table_name'], $input['object_id'], $input['price']);
        if (!$data) {
            $this->error('收藏失败');
        }
     ...
     */
    protected function _FavoritesObject($title, $url, $description, $table_name, $object_id, $price)
    {
