...
class GoodsPostModel extends CommonModel
{
    //可查询字段
    protected $visible = [
        ...
        'user', 'category_id', 'post_price','post_store'
    ];
    ...
    {
        return $this->belongsTo('api\goods\model\UserModel', 'user_id');
    }
    ...
    {
        return $this->belongsTo('api\goods\model\UserModel', 'user_id')->field('id,user_nickname');
    }
    ...
    {
        return $this->belongsToMany('api\goods\model\GoodsCategoryModel', 'goods_category_post', 'category_id', 'post_id');
    }
    ...
    {
        return $this->belongsToMany('api\goods\model\GoodsTagModel', 'goods_tag_post', 'tag_id', 'post_id');
    }
    ...
    {
        return $this->hasOne('api\goods\model\RecycleBinModel', 'object_id');
    }
    ...
    /**
     * 根据商品关键字，增加标签
     * @param array $keywords 商品关键字数组
     * @param int $articleId 商品id
     * @return void
     */
        ...
        if (!empty($keywords) || !empty($names)) {
            if (!empty($names)) {
                ...
                if (!empty($shouldDeleteNames)) {
                    ...
                    $tagPosts   = DB::name('goods_tag_post')->where('tag_id', 'in', $tagIds)
                    ...
                    DB::name('GoodsTag')->delete($shouldDeleteTagIds);
                    DB::name('GoodsTagPost')->delete($tagPostIds);
                }
            } else {
                $tagIdNames = DB::name('goods_tag')->where('name', 'in', $keywords)->column('name', 'id');
                if (!empty($tagIdNames)) {
                    $tagIds = array_keys($tagIdNames);
    ...
    /**
     * 删除商品
     * @param $ids  int|array   商品id
     * @param int $userId 商品所属用户id  [可选]
     * @return bool|int 删除结果  true 成功 false 失败  -1 商品不存在
     */
    ...
    /**
     * 过滤属于当前用户的商品，超级管理员除外
     * @params  array $ids     商品id的数组
     * @param   int $userId 当前用户id
     * @return  array     属于当前用户的商品id
     */
