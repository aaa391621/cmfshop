...
class ArticleController extends HomeBaseController
{
    public function index()
    {
        ...
        if (empty($categoryId)) {
            ...
            $category = $goodsCategoryModel->where('id', $categoryId)->where('status', 1)->find();
        ...
        Db::name('goods_post')->where(['id' => $articleId])->setInc('post_hits');


        hook('goods_before_assign_article', $article);
    ...
    public function doLike()
    {
        ...
        if ($canLike) {
            Db::name('goods_post')->where(['id' => $articleId])->setInc('post_like');
