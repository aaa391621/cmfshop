...
class GoodsPostModel extends Model
{
    ...
    {
        return $this->belongsToMany('GoodsCategoryModel', 'goods_category_post', 'category_id', 'post_id');
    }
    ...
    {
        return $this->belongsToMany('GoodsTagModel', 'goods_tag_post', 'tag_id', 'post_id');
    }
    ...
    public function addTags($keywords, $articleId)
    {
        ...
        if (!empty($keywords)) {
            ...
            $oldTagIds = Db::name('goods_tag_post')->where('post_id', $articleId)->column('tag_id');

            foreach ($keywords as $keyword) {
                $keyword = trim($keyword);
                if (!empty($keyword)) {
                    $findTag = $goodsTagModel->where('name', $keyword)->find();
                    if (empty($findTag)) {
            ...
            if (empty($tagIds) && !empty($oldTagIds)) {
                Db::name('goods_tag_post')->where('post_id', $articleId)->delete();
            }
            ...
            if (!empty($shouldDeleteTagIds)) {
                Db::name('goods_tag_post')->where(['post_id' => $articleId, 'tag_id' => ['in', $shouldDeleteTagIds]])->delete();
            }

            if (!empty($data)) {
                Db::name('goods_tag_post')->insertAll($data);
            }
    ...
    public function adminDeletePage($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            if ($res) {
                ...
                $transStatus = false;
                try {
                    Db::name('goods_post')->where(['id' => $id])->update([
                        'delete_time' => time()
                    ]);
            ...
            if ($res) {
                ...
                foreach ($res as $key => $value) {
                    ...
                    $recycleData[$key]['table_name']  = 'goods_post';
                ...
                $transStatus = false;
                try {
                    Db::name('goods_post')->where(['id' => ['in', $ids]])
                        ->update([
                            'delete_time' => time()
                        ]);
    ...
    public function adminEditPage($data)
    {
        ...
        $routeModel->setRoute($data['post_alias'], 'goods/Page/index', ['id' => $data['id']], 2, 5000);
